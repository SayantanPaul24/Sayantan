...
dim(X)
beta<-c(rep(4,4),rep(0,4),rep(0,2))
...
inits<- function(){
  list(beta1=c(4.1,4.2),beta2=c(4.15,4.25),beta3=c(2.01,2.24),beta4=c(2.23,2.11),beta5=c(0.05,0.04),k=c(0.3792589886,0.9000829897,0.3894594,0.5986663, 0.9641334),gamma=0.4561125)
}
...
X<- cbind(X1,X2,X3,X4,X5)
dim(X)
beta<-c(0.1,0.2,0.3,0.4,rep(0,16))
...
X<- cbind(x1,x2,x3,x4,x5)
dim(X)
beta<-c(0.8,0,0,0,0.1,0.2,0.3,0.4,rep(0,12))
...
inits<- function(){
  list(beta1=c(0.81,0.12,0.03,0.04),beta2=c(0.05,0.15,0.2,0.35),beta3=c(0.01,0.24,0.04,0.21),beta4=c(-0.23,-0.41,-0.04,-0.13),beta5=c(0.05,0.54,0.03,0.17),
       k=c(0.3792589886,0.9000829897,0.3894594,0.5986663, 0.9641334),gamma=0.90417050)
}
...
Modi2.sim <- bugs(data_4, inits, model.file = "C:/Users/Sayantan/Documents/Usual Group Horseshoe p=20.bug",
                  parameters = c("beta1", "beta2", "beta3","beta4","beta5"),
                  n.chains = 1, n.iter = 10000,n.sims = 3000, bugs.directory = "C:/Users/Sayantan/Downloads/winbugs143_unrestricted/winbugs14_full_patched/WinBUGS14/",DIC = FALSE)
...
round(betahat,digits=4)

print(Modi1.sim$mean$beta1)
...
dim(X)
beta<-c(2,0,0,0,rep(0,18),rep(2,4),rep(0.4,1),0,0,0,rep(0,4),rep(1.5,4),rep(0,2))
...
betahat10<- solve(t(x10)%*%x10)%*%t(x10)%*%y
betahat11<- solve(t(x11)%*%x11)%*%t(x11)%*%y
betahat13<- solve(t(x13)%*%x13)%*%t(x13)%*%y
...
X<- cbind(x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14)
dim(X)
beta<-c(2,0,0,0,rep(0,18),rep(2,4),rep(0.4,4),rep(0,4),rep(1.5,4),rep(0,2))
